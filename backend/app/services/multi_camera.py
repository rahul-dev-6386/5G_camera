...
def _similarity_score(candidate: dict, existing: dict) -> float:
    ...
    return spatial_score * 0.4 + appearance_score * 0.34 + size_score * 0.16 + confidence_score * 0.1


def _track_merge_score(left: dict, right: dict) -> float:
    anchor_distance = float(np.linalg.norm(left["anchor"] - right["anchor"]))
    histogram_similarity = cv2.compareHist(left["histogram"], right["histogram"], cv2.HISTCMP_CORREL)
    histogram_similarity = max(min(float(histogram_similarity), 1.0), -1.0)
    appearance_score = max(0.0, (histogram_similarity + 1.0) / 2.0)
    spatial_score = max(0.0, 1.0 - anchor_distance / 0.24)
    size_gap = abs(left["size"][0] - right["size"][0]) + abs(left["size"][1] - right["size"][1])
    size_score = max(0.0, 1.0 - size_gap / 0.4)
...
def fuse_camera_detections(camera_results: list[dict]) -> dict:
    ...
    for index, camera_result in enumerate(camera_results):
        ...
        for detection in result["detections"]:
            if float(detection["confidence"]) < 0.38:
                continue
            bbox = detection["bbox"]
    ...
    for candidate in candidates:
        ...
        if best_match_index is not None and best_score >= 0.56:
    ...
    # Merge any residual highly-similar tracks to reduce duplicate counts across overlapping views.
    merged_tracks: list[dict] = []
    for track in fused_tracks:
        merged = False
        for existing in merged_tracks:
            if set(track["camera_ids"]).intersection(existing["camera_ids"]):
                continue
            if _track_merge_score(track, existing) >= 0.6:
                existing["camera_ids"].extend(track["camera_ids"])
                existing["anchor"] = (existing["anchor"] + track["anchor"]) / 2
                existing["size"] = (existing["size"] + track["size"]) / 2
                existing["histogram"] = (existing["histogram"] + track["histogram"]) / 2
