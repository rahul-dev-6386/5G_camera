...
class StreamManager:
    ...
    def _read_frame(self, user_id: str, camera_id: str, session: CameraSession) -> bytes:
        ...
        if not success:
            ...
            session.health_status = "degraded"
            self.logger.warning(
                f"Failed to read frame from {camera_id} (attempt {session.failed_reads}/{self._max_failed_reads})"
            )
            
            if session.failed_reads >= self._max_failed_reads:
                session.capture.release()
                self.logger.error(f"Max failed reads reached for {camera_id}, attempting reconnection")
                ...
                with self._lock:
                    ...
                    if current is session:
                        ...
                        self.logger.info(f"Successfully reconnected to stream {camera_id}")
                    else:
                        replacement.release()
                        raise StreamSessionError(f"The stream session for '{camera_id}' changed while reconnecting.")
        else:
            # Reset failed reads on success
    ...
    def check_camera_health(self, user_id: str) -> dict[str, str]:
        """Check health status of all cameras for a user."""
        with self._lock:
            sessions = dict(self._sessions.get(user_id, {}))
        ...
        for camera_id, session in sessions.items():
            ...
            if time_since_last_frame > self._health_check_interval:
                health_status[camera_id] = "stale"
                session.health_status = "stale"
                self.logger.warning(f"Camera {camera_id} has stale frames ({time_since_last_frame:.1f}s since last frame)")
            elif session.failed_reads > 0:
                health_status[camera_id] = "degraded"
            else:
                health_status[camera_id] = "healthy"
